...
//~ ,
    //~ ,
     //~ { desc:"huge magnitude with gaps and duplicates and infinites",
      //~ func:function(len){ 
        //~ return Fdrandom.mixof( 
          //~ Fdrandom.bulk( 
           ...
           //~ ,function(){ 
             ...
             //~ return g*g*g*g*g*g*g*Fdrandom.range(-500000,500000)
            //~ }
          //~ ) 
          //~ ,len 
      //~ ) } }
    ...
    //~ { desc:"cauchy distributed ints from -50 to 50",
      //~ func:function(len){ return Fdrandom.bulk( len,function(){ return Math.floor(Fdrandom.gcauchy(-50,50))} )  } }
    ...
    for(var ttin in tlens){
      ...
      while(loops--){
        ...
        if(sortchecking){
          ...
          else{ conlog("Length",tzA.length+" - tested:"); conlog(rslt) }
          
          if(sortchecking2){
            conlog("The out:")
            var ttt=Barsort.reorder(tzA,nim)
            conlog(joinendsfixedwidth(ttt,ttt[3]===Math.floor(ttt[3])?0:5))
          }
        }
...
function sortcheck(valz,sdic,nopre,odesc,st,ov){ //st,ov not implemented
  ...
  var chex=[]; for(var i=0;i<valz.length;i++) chex[i]=i
  Barsort.longsort(valz,chex,100)
  ...
  for( var ord=0,e=valz.length; ord<e; ord++ ){
    ...
    if(!skiplog) console.log("mispos ord",ord,"sdic",sdic[ord],"chex",chex[ord],"vals",valz[sdic[ord]],valz[chex[ord]])
    dentical=false
...
function joinendsfixedwidth(Ar,fx){
  ...
  var s=zap(Ar.slice(0,en),function(a){return a.toFixed(fx)}).join(", ")
      + ",...," + zap(Ar.slice((an-en)>>1,(an+en)>>1),function(a){return a.toFixed(fx)}).join(", ")
      + ",...," + zap(Ar.slice(an-en,an),function(a){return a.toFixed(fx)}).join(", ")
...
function zap(Ai,f){
  ...
  function ntain(a,b,c){ return a<b?b:c<a?c:a }
