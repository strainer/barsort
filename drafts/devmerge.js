...
Fdr=Fdrandom=require ('../dlib/Fdrandom.js').hot()
...
var see2=false//true//false//true//false
...
function web(len){ var ll=0; 
  return Fdrandom.bulk( len,function(){ return (ll++)+Fdrandom.irange(-len/100,len/100)*(Math.sin(ll/len*7)+Math.sin(ll/len*8)+Math.sin(ll/len*18)+Math.sin(ll/len*9)) } ) 
  }
...
var zlen=10000 //broke

var Av=web(zlen), Alen=Av.length, Ax=[]
...
function sortcheck(valz,sdic,nopre,odesc,st,ov){ //st,ov not implemented
  ...
  function longsort(Av,Ax,bottle){
    ...
    while ( st < Alen){ 
      ...
      if(snip||(top===btop)){ 
        ...
        var k=sparsemerge(Av,Ax,st-tail,nx,uflow )
  ...
  function minglemerge(Av,Ax,s,e,b){
    ...
    while(clonx>=0)// ix of copyel to place
    {
      ...
      //~ if( (hipt>=b) && compar( Av[Ax[hipt]],Av[submcach[clonx]] ) ){
        //~ hipt--
      while( (hipt>=b) && compar( Av[Ax[hipt]],Av[submcach[clonx]] ) ) // is not jdest
      { hipt-- } 
